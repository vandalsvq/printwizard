...
`pw_public` — **публичный репозиторий документации** PrintWizard (Конструктор печатных форм), расширения 1С:Предприятия для разработки печатных форм. Здесь лежит **только документация и инструменты её преобразования**. Сам продукт (код 1С/BSL) — в отдельном приватном репозитории; упоминаемые здесь спецификации (например, `specs/pw-217/spec.md`) ссылаются туда.
...
## Задачи на документацию — в приватном репозитории продукта

Задачи на документацию продукта ведутся в приватном репозитории продукта, под двумя метками — «Документация» и «Агент документации». В этом публичном репозитории они больше не заводятся: строка вида «форма X изменилась во внутренней задаче N» — внутренняя кухня, а публичный репозиторий остаётся витриной.

- **Заводит** агент разработки при закрытии своей задачи (шаг 2 скилла `pw-release`); документационная сессия заводит задачи себе сама — там же и с теми же двумя метками.
- **Закрывает** документационная сессия — после merge правок в `master` этого репозитория, комментарием со ссылкой на коммит. В сообщениях коммитов на приватные задачи не ссылаемся: связь фиксируется комментарием в самой задаче.
- Внутреннее (ссылки на внутренние задачи, спеки, мотивы) допустимо в теле задачи, но не в тексте документации и не на сайте.
- Гранулярность, milestone, порядок со скриншотами и адрес трекера — в CLAUDE.md репозитория продукта; у сессии адрес и команда поиска работы лежат в памяти, а не в публичном файле.
