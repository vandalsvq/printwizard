...
`### TOPIC: <key> ###` на отдельной строке, сразу за ним — однострочная
аннотация темы `### SUMMARY: <текст> ###`. Тело секции — до следующего
маркера темы или до конца файла.

Аннотация нужна рантайму: по одному имени ключа модель не понимает, что
внутри темы, поэтому `PW_GetDocs("список")` и выдача `PW_SearchDocs`
показывают её вместе с ключом.
