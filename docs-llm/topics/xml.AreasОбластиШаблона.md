...
|---|---|
| `PrintMethod` | Способ вывода — см. перечисление (см. topic: xml.Перечисления) |
| `PrintSettings` | Дополнительная настройка — см. перечисление (см. topic: xml.Перечисления) |
...
| Значение `Type` | В конструкторе | Описание | Тип значения `<Value>` |
|---|---|---|---|
| `Dataset` | `ЗначениеНабора` «Значение набора» | Поле из набора данных | `Field.Dataset` |
| `Description` | `КонструкторСтроки` «Конструктор строки» | Составное представление | `Field.Description` |
| `Algorithm` | `ПроизвольныйАлгоритм` «Алгоритм» | Произвольный код 1С | `Field.Function` |
| `SumInWords` | `СуммаПрописью` «Сумма прописью» | Сумма прописью | `Field.SumInWords` |
| `QRCode` | `ДанныеQRКода` «Данные QR-кода» | QR-код | `Field.QRCode` |
...
| `ComparisonType` | Вид сравнения — см. перечисление (см. topic: xml.Перечисления) |
