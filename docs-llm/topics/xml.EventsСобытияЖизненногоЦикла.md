...
Элемент `Events` содержит единственный дочерний элемент `<Algorithm>` с кодом на встроенном языке 1С. Допустимые значения атрибута `Name` — см. перечисление (см. topic: xml.Перечисления).
